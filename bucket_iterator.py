...
class BucketIterator(object):
    ...
    def pad_data(self, batch_data):
        ...
        for item in batch_data:
            context, aspect, text_indices, context_indices, aspect_indices, aspects_indices, left_indices,\
            polarity, sentic_graph, sdat_graph, aspect_graph = \
                item['context'], item['aspect'], \
                item['text_indices'], item['context_indices'], item['aspect_indices'], item['aspect_indices'], item['left_indices'],\
                item['polarity'], item['sentic_graph'], item['sdat_graph'], item['aspect_graph']
            text_padding = [0] * (max_len - len(text_indices))
            context_padding = [0] * (max_len - len(context_indices))
            aspect_padding = [0] * (max_len - len(aspect_indices))
            aspects_padding = [0] * (max_len - len(aspects_indices))
            ...
            batch_aspects_indices.append(aspects_indices + aspects_padding)
        ...
        return { \
                ...
                'aspects_indices': torch.tensor(batch_aspects_indices), \
